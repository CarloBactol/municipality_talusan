    ...
    <div class="container">
        <nav aria-label="breadcrumb" class="my-3">
          ...
          <div class="row d-flex justify-content-center">
            <div class="col-lg-4 ">
              <form action="{{ route('frontend.news.february') }}" method="GET" enctype="multipart/form">
                <div class="row">
                    <div class="col-lg-12">
          ...
          <div class="row">
            ...
            <div class="col-lg-4 col-md-6 col-sm-12 my-lg-4 d-flex justify-content-center my-sm-2">
                ...
                <img src="{{ asset('assets/uploads/news/'.$feb->image) }}" class="card-img-top" alt="..." height="200">
                <div class="card-body">
                    <h6 class="card-title">{{ $feb->title }}</h6>
                    <small>{{ \Carbon\Carbon::parse($feb->created_at)->isoFormat('MMM Do YYYY') }}</small> |
                    <small>{{ \Carbon\Carbon::parse($feb->created_at)->diffForHumans() }}</small>
                    <small>{!! Str::limit($feb->description, 70) !!}</small>
                    <div class="card-footer"><a href="{{ url('news-update/'.$feb->id) }}">Read Full</a></div>
                </div>
                </div>
            </div>
            @endforeach
