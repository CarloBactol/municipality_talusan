    ...
    <div class="container">
        <nav aria-label="breadcrumb" class="my-3">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active" aria-current="page">City Mayor</li>
            </ol>
          </nav>
          ...
          <div class="row">
            @foreach ($city_mayors as $city_mayor)
            <div class="col-lg-4 col-mg-4 col-12 d-flex justify-content-center my-md-2">
                <div class="card" style="width: 18rem;">
                    <img src="{{ asset('assets/uploads/cityCouncil/'.$city_mayor->image ) }}" class="card-img-top" alt="..." height="200">
                    <div class="card-body">
                       <h5>{{ $city_mayor->name }} <span class="badge bg-success">{{ $city_mayor->type }}</span></h5>
                       <small class="text-info mr-2">Date Elected:</small> <span>{!! Carbon\Carbon::parse($city_mayor->date_elected)->isoFormat('MMM Do YYYY') !!}</span>
                       <div class="row mt-2 mb-0">
                         <a href="{{ url('about/profile/'.$city_mayor->id) }}" class="btn btn-outline-success">View Profile</a>
                       </div>
                    </div>
                </div>
            </div>       
